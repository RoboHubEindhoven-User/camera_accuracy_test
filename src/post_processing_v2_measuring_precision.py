...
class PostProcessing:
    ...
    def __init__(self):
        ...
        """     
        #rospy.init_node("vision_marker", anonymous=True)
        self.br = tf2_ros.TransformBroadcaster()
        self.left_upper = []
        self.right_lower = []
        # Measuring square to measure mm/pixel ratio
        l_square = 40 #mm
        w_square = 40 #mm
    ...
    def build_center(self, object_name, roi, img, debug, measuring_name):
        ...
        """   
        self.cx_biggest = None
        self.measuring_name = str(measuring_name)   
    ...
    def calculate_draw_result(self,img):
        ...
        if self.cx_biggest is not None:
            ...
            x_orientation = x_orientation*self.l_per_pix #calculate to mm
            y_orientation = y_orientation*self.w_per_pix #calculate to mm
            ...
            self.pub_tf(x_orientation,y_orientation,myradians)
            return x_orientation,y_orientation,myradians,True 
        else:
            return 0,0,0,False

    def pub_tf(self,x ,y, theta):
        """[Broadcasts object tf]
        
        Arguments:
            x {[float]} -- [X coordinate of the middlepoint of the object]
            y {[float]} -- [Y coordinate of the middlepoint of the object]
            theta {[float]} -- [Theta of the middlepoint of the object]
        """
        transform_msg = geometry_msgs.msg.TransformStamped()
        transform_msg.header.stamp = rospy.Time.now()
        transform_msg.header.frame_id = "camera"
        transform_msg.child_frame_id = self.measuring_name
        ...
        transform_msg.transform.translation.x = (x/1000) * 1
        transform_msg.transform.translation.y = (y/1000) * 1
        transform_msg.transform.translation.z = -0.4

        # Recalculate theta from radians to quaternions
